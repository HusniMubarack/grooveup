...
export function AuthForm({ mode, next, defaultRole }: { mode: "login" | "register"; next?: string; defaultRole?: string }) {
  ...
  return (
    <form action={action} className="mx-auto mt-6 grid max-w-sm gap-4 rounded-xl border bg-card p-5">
      <h1 className="font-serif text-2xl">{mode === "login" ? "Welcome back" : "Join Atelier"}</h1>
      {mode === "register" && (
        <div className="grid gap-1"><Label htmlFor="name">Name</Label><Input id="name" name="name" required autoComplete="name" /></div>
      )}
      <div className="grid gap-1"><Label htmlFor="email">Email</Label><Input id="email" name="email" type="email" required autoComplete="email" /></div>
      ...
      {mode === "register" && (
        <div className="grid gap-1">
          ...
          <Select id="role" name="role" defaultValue={defaultRole ?? "STUDENT"}>
      ...
      <input type="hidden" name="next" value={next ?? "/explore"} />
      ...
      {mode === "login" && (
        <p className="text-center text-[11px] text-muted-foreground">Demo: teacher@ / student@ / admin@atelier.dev · password123</p>
      )}
    </form>
  );
}
